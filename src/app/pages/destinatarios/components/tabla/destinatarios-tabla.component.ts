...
@Component({
  selector: 'app-destinatarios-tabla',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './destinatarios-tabla.component.html',
  styleUrl: './destinatarios-tabla.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DestinatariosTablaComponent {
  private readonly _items = signal<Destinatario[]>([]);

  @Input({ required: true })
  set items(v: Destinatario[]) {
    this._items.set(Array.isArray(v) ? v : []);
    this.page.set(1);
  }
  get items(): Destinatario[] {
    return this._items();
  }

  @Output() editar = new EventEmitter<Destinatario>();
  @Output() toggleActivo = new EventEmitter<Destinatario>();

  @Input()
  showToggleActivo = true;
  ...
  onEditar(item: Destinatario): void {
    this.editar.emit(item);
  }

  onToggleActivo(item: Destinatario): void {
    this.toggleActivo.emit(item);
  }

  trackById = (index: number, item: Destinatario) => (item as any)?._pk ?? item?.id ?? item?.idCliente ?? index;
