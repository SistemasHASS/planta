...
const COLUMNAS = [
  ...
  { campo: 'documento', label: 'Documento', tipo: 'nvarchar' },
  { campo: 'nombre', label: 'Razón Social', tipo: 'nvarchar' },
  { campo: 'nombreGrupoCliente', label: 'Grupo Cliente', tipo: 'nvarchar', maxLength: 200, required: true, editable: false },
  { campo: 'codigosRancho', label: 'Códigos Rancho', tipo: 'virtual', visible: true },
  { campo: 'bd', label: 'Sincronizado', tipo: 'bit', visible: true },
];

@Component({
  selector: 'app-catalogo-consignatarios-tabla',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './catalogo-consignatarios-tabla.component.html',
  styleUrl: './catalogo-consignatarios-tabla.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CatalogoConsignatariosTablaComponent {
  ...
  readonly codigosRanchoModalItem = signal<any>(null);

  getCodigosRanchoTexto(item: any): string {
    const lista = (item?.codigosRancho ?? []).filter((c: any) => c?.activo === true || c?.activo === 1);
    if (lista.length === 0) return '-';
    const texto = lista.map((c: any) => String(c?.codigo ?? '')).filter(Boolean).join(', ');
    return texto;
  }

  truncarCodigosRancho(texto: string): { texto: string; truncado: boolean } {
    if (texto.length <= 20) return { texto, truncado: false };
    return { texto: texto.slice(0, 20) + '...', truncado: true };
  }

  getCodigosRanchoActivos(item: any): any[] {
    const lista = item?.codigosRancho ?? [];
    return lista.filter((c: any) => c?.activo === true || c?.activo === 1);
  }

  abrirModalCodigosRancho(item: any, event: Event): void {
    event.stopPropagation();
    this.codigosRanchoModalItem.set(item);
  }

  cerrarModalCodigosRancho(): void {
    this.codigosRanchoModalItem.set(null);
  }
}
